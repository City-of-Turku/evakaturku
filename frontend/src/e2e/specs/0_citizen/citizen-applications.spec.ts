...
const customerContactText = 'Varhaiskasvatuksen asiakaspalveluun: varhaiskasvatus@turku.fi / 08 558 45300 - valitse numero 1 (ma-pe klo 9-12).'
const customerContactEmailHref = 'mailto:varhaiskasvatus@turku.fi'
...
describe('Citizen applications page', () => {
  ...
  test('Daycare application form customizations', async () => {
    ...
    await waitUntilEqual(
      ...
      'Lisätietoa varhaiskasvatuksen asiakasmaksuista löydät Turun kaupungin sivuilta '
    )
    await waitUntilEqual(
  ...
  test('Club application form customizations', async () => {
    ...
    await waitUntilEqual(
      ...
      'Jos lapsella on tuen tarve, Turun varhaiskasvatuksesta otetaan yhteyttä hakemuksen jättämisen jälkeen.'
    )
    await editorPage.openSection('contactInfo')
    await waitUntilEqual(
