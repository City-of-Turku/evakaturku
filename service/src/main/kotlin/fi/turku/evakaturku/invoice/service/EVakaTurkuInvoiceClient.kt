...
import fi.turku.evakaturku.invoice.config.findProduct
...
class EVakaTurkuInvoiceClient(
    ...
    private fun toRequest(invoices: List<InvoiceDetailed>): SendSalesOrderRequest {
        return SendSalesOrderRequest().apply {
            salesOrder = SalesOrder().apply {
                header.addAll(invoices.map(this@EVakaTurkuInvoiceClient::toHeader))
            }
        }
    }
    ...
    private fun toItems(rows: List<InvoiceRowDetailed>): Items {
        return Items().apply {
            item.addAll(rows.map(this@EVakaTurkuInvoiceClient::toItem))
        }
    }
