...
package fi.turku.evakaturku.export
...
class PreschoolTransferDocumentReportTest : AbstractIntegrationTest() {
    ...
    private val areaId = AreaId(UUID.fromString("3d21700a-9558-11ec-9cd5-43d5752dad51"))
    private val unit = DevDaycare(areaId = areaId, name = "Ankkalinnake")
    private val employee = DevEmployee()

    private val templateNameFi = "Tiedonsiirto esiopetuksesta perusopetukseen koulupaikkapäätöksen suunnittelun tueksi ja opetuksen järjestämiseksi."
    private val templateNameSv = "Överföring av uppgifter från förskoleundervisningen till den grundläggande utbildningen som stöd för beslut om skolplacering och för ordnandet av undervisningen."
    ...
    fun `child with completed transfer document and preschool placement is included`() {
        ...
        val rows = db.read { tx ->
            tx.createQuery { sql("SELECT * FROM preschool_transfer_document_report(${bind(date)})") }
        ...
        db.transaction { tx ->
            ...
            tx.insert(
                DevPreschoolAssistance(
                    ...
                    modifiedBy = EvakaUser(employee.evakaUserId, employee.firstName + " " + employee.lastName, EvakaUserType.EMPLOYEE),
                ),
            )
        }
    ...
    fun `child with multiple support levels`() {
        db.transaction { tx ->
            ...
            val modifiedBy = EvakaUser(employee.evakaUserId, employee.firstName + " " + employee.lastName, EvakaUserType.EMPLOYEE)
            tx.insert(
                DevPreschoolAssistance(
                    childId = childId,
                    validDuring = FiniteDateRange(date, date),
    ...
    fun `child without completed document is not included`() {
        db.transaction { tx ->
            val childId = tx.insert(
                DevPerson(dateOfBirth = LocalDate.of(2019, 1, 1)),
                DevPersonType.CHILD,
            )
            tx.insert(
                DevPlacement(
